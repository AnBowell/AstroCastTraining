...
def Merge(ToMerge, file_dst):
    ...
    OutputName = str(Names[0]) + '_' + str(Names[1]) +'_' +str(Names[-2]) #This will need to be changed if file dest changes

    
    with rasterio.open(file_dst +str(OutputName) +'.tif', "w",**out_meta) as dest:
        dest.write(mosaic)